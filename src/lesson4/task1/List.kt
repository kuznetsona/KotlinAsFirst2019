...
fun russian0(p: Int, k: Int): String {
    ...
    if (p % 100 < 10 || p % 100 >= 20) {
        if (counter >= 2 && p % 100 / 10 != 0) {
            str += russian10(p % 100 / 10)
        }
        if (p % 10 != 0) {
...
fun russian10(p: Int): String {
    ...
    str += when (p) {
        ...
        4 -> " сорок"
        9 -> " девяносто"
        else -> russian1(p) + "десят"
    }
    return str
}

fun russian1(p: Int): String {
    var str = ""
    str += when (p) {
        1 -> " один"
        2 -> " два"
